...
prompt_template = PromptTemplate.from_template(
    ...
    대화의 흐름을 자연스럽게 이어가고, 내가 보낸 이모지에 대해 진심으로 공감해줘.
    ...
    어울릴만한 음식 1가지를 추천해주고, 음식에 대해 두 문장 정도로 간단한 이유도 덧붙여줘.
    각 음식 이름은 이름만 제시하고, 절대 이모지로 표현하지 마.

    형식:
    1. 음식이름 - 이유

    절대로 이 형식을 벗어나지 마. 문단이나 말풍선 형태, 자연어 형식으로 풀어내지 마. 꼭 숫자. 음식 - 이유 형태로 출력해줘.
    """
)
